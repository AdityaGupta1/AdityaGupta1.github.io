/**
 * Writes assets/js/albums-manifest.js from assets/data/albums.json.
 ...
 *
 * Run after editing albums.json: node scripts/sync-albums-manifest.mjs
 * Optional: node scripts/sync-albums-manifest.mjs --write-json
 *   rewrites albums.json (same schema: images stay number[] only).
 */
import fs from "fs";
...
const body =
  "/* Generated by scripts/sync-albums-manifest.mjs — edit assets/data/albums.json then run: node scripts/sync-albums-manifest.mjs */\n" +
  "window.__ALBUMS_MANIFEST__ = " +
...
function manifestToSourceAlbumsJson(merged) {
  return {
    photoPaths: merged.photoPaths,
    featuredIds: merged.featuredIds,
    albums: merged.albums.map((a) => ({
      id: a.id,
      title: a.title,
      date: a.date,
      cover: a.cover,
      images: a.images.map((img) => img.file),
    })),
  };
}

if (writeJson) {
  const slim = manifestToSourceAlbumsJson(merged);
  const jsonOut = JSON.stringify(slim, null, 2) + "\n";
  fs.writeFileSync(jsonPath, jsonOut, "utf8");
  console.log("Updated", path.relative(root, jsonPath), "(--write-json)");
}
